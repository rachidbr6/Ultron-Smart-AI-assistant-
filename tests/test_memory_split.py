...
import ultron.memory.memory_habits as memory_habits
import ultron.memory.memory_notes as memory_notes
import ultron.memory.memory_preferences as memory_preferences
import ultron.memory.memory_short_term as memory_short_term
...
class MemorySplitTests(unittest.TestCase):
    ...
    def test_track_command_updates_top_habits(self):
        ...
        with patch("ultron.memory.memory_habits.load_memory", return_value=memory), patch("ultron.memory.memory_habits.save_memory") as save_mock:
            memory_habits.track_command("open chrome")

        save_mock.assert_called_once()

    def test_preferences_and_notes_are_routed(self):
        with patch("ultron.memory.memory_preferences.set_preference") as set_pref:
        ...
        with patch("ultron.memory.memory_notes.load_memory", return_value={"notes": []}), patch("ultron.memory.memory_notes.save_memory") as save_mock:
