...
def _low_latency_kwargs(model: str) -> dict[str, Any]:
    """Cut hidden reasoning-token overhead for gpt-oss models.

    Default/medium reasoning effort on this model family is both slower and
    prone to spending its whole token budget on hidden reasoning, returning
    empty content. Low effort measured 2-3x faster and fully reliable.
    """
...
class GroqProvider:
    ...
    def analyze(self, request: ProviderRequest) -> ProviderResponse:
        ...
        try:
            ...
            response = active_client.chat.completions.create(
                ...
                temperature=0.1,
                max_tokens=500,
                **_low_latency_kwargs(self.model),
            )
    ...
    def translate(self, text: str, *, target_language: str = "English") -> ProviderResponse:
        if not self.enabled or not self.api_key:
            return ProviderResponse(provider=self.name, status="unavailable", error="Groq provider is unavailable.")
        try:
            active_client = self._client()
                ...
                messages=[
                    {
                        "role": "user",
                        "content": (
                            f"Translate the following text to natural, fluent {target_language}. "
                            f"It will be read aloud by a {target_language} text-to-speech voice, so "
                            f"translate every word into {target_language} - including section or category "
                            f"titles - and do not leave any words in the source language, since those "
                            "would be mispronounced. Preserve dates, numbers, and overall structure. "
