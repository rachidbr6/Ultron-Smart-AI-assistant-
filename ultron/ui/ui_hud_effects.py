"""Premium HUD canvas effects for the ULTRON desktop UI."""
...
def draw_background_depth(canvas, *, palette: dict) -> None:
    ...
    canvas.delete("all")
    # Handle canvas width/height which may include unit suffixes like '10c'
    width_val = canvas["width"] or canvas.winfo_width() or 1400
    height_val = canvas["height"] or canvas.winfo_height() or 760
    
    # Strip non-numeric suffixes (e.g., '10c' -> 10)
    if isinstance(width_val, str):
        width_val = ''.join(c for c in width_val if c.isdigit() or c == '.')
        width = int(float(width_val)) if width_val else 1400
    else:
        width = int(width_val)
        
    if isinstance(height_val, str):
        height_val = ''.join(c for c in height_val if c.isdigit() or c == '.')
        height = int(float(height_val)) if height_val else 760
    else:
    ...
    cx = int(width * 0.60)
    cy = int(height * 0.31)
    for radius, color in [(360, "#1a0508"), (300, "#210609"), (240, "#2c090d"), (180, "#1c0a10")]:
        canvas.create_oval(cx - radius, cy - radius, cx + radius, cy + radius, outline=color, width=1)
    for y in range(14, height, 28):
        canvas.create_line(0, y, width, y, fill="#1a0509", width=1)
    for y in range(0, height, 5):
        canvas.create_line(0, y, width, y, fill="#150609", width=1)
    ...
    for side in range(5):
        alpha_color = ["#120609", "#14060b", "#17070d", "#1b070f", "#1e0710"][side]
...
def draw_active_sidebar_icon(canvas, *, palette: dict) -> None:
    ...
    canvas.delete("all")
    canvas.create_rectangle(2, 2, 58, 58, outline="#5c1620", width=3)
    canvas.create_rectangle(6, 6, 54, 54, fill=panel, outline=accent, width=1)
    canvas.create_oval(16, 16, 44, 44, outline="#7a0f1f", width=5)
...
def draw_sidebar_nav_icon(canvas, icon: str, *, palette: dict, active: bool = False, hover: bool = False) -> None:
    ...
    canvas.delete("all")
    if active or hover:
        outer = palette["cyan_soft"] if hover else "#5c1620"
...
def draw_mini_orb(canvas, size: int, *, palette: dict, phase: float = 0.0) -> None:
    ...
    canvas.create_oval(8, 8, size - 8, size - 8, outline="#4d0616", width=1)
...
def draw_equalizer(canvas, phase: float, *, palette: dict, activity: float = 1.0) -> None:
    ...
    for index in range(18):
        ...
        canvas.create_rectangle(x - 1, 64 - value - 2, x + 4, 64, fill="#4a0512", outline="")
...
def draw_waveform(canvas, phase: float, *, palette: dict, activity: float = 1.0) -> None:
    ...
    for x in range(16, width, 24):
        canvas.create_line(x, 5, x, height - 5, fill="#340b13", width=1)
    for y in range(12, height, 12):
        canvas.create_line(5, y, width - 5, y, fill="#340b13", width=1)
    points = []
    baseline = height // 2
    for index in range(36):
        ...
        points.extend([x, y])
    canvas.create_line(points, fill="#67071e", width=4, smooth=True)
