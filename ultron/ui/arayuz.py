"""ULTRON desktop UI - cyber hologram interface."""
...
from ultron.ui.ui_hud_effects import (
...
class JarvisApp(ctk.CTk):
    ...
    def _build_center_stage(self):
        ...
        ctk.CTkLabel(title_stack, text="U.L.T.R.O.N", font=font("display", 18, "bold"), text_color=TEXT).pack(anchor="w")
        ...
        self._subtitle_label = ctk.CTkLabel(
            self._dashboard_page,
            text="I am a virtual assistant ULTRON, how may I help you?",
            font=font("mono", 14),
            text_color=TEXT_DIM,
        )
        ...
        self._add_log("ULTRON initialized", "info")
    ...
    def _update_ui(self, message: str):
        ...
        clean_message = message.replace("JARVIS:", "ULTRON:").strip()
        ...
        if "ultron:" in lowered or "[cmd]" in lowered or "you:" in lowered:
    ...
    def _build_tray_image(self):
        ...
        draw.ellipse((2, 2, size - 2, size - 2), fill="#150808", outline=BLUE, width=3)
        draw.text((size / 2, size / 2), "U", fill=BLUE, anchor="mm")
        return image

    def _setup_tray_icon(self):
        menu = pystray.Menu(
            pystray.MenuItem("Show Ultron", self._show_from_tray, default=True),
            pystray.MenuItem("Quit Ultron", self._quit_app),
        )
        self._tray_icon = pystray.Icon("ultron", self._build_tray_image(), "U.L.T.R.O.N", menu)
        threading.Thread(target=self._tray_icon.run, daemon=True).start()

    def _hide_to_tray(self):
        self.withdraw()
        self._add_log("Minimized to system tray. Say \"ultron\" any time.", "info")
