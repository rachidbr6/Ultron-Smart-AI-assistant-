...
from ultron.ui.ui_hud_effects import draw_sidebar_nav_icon
from ultron.ui.ui_theme import PALETTE
...
def build_sidebar(parent, nav_canvases: dict, on_select, on_hover) -> tuple[ctk.CTkFrame, ctk.CTkCanvas]:
    ...
    sidebar = ctk.CTkFrame(parent, fg_color="#1C0709", corner_radius=0, width=90, border_width=1, border_color=PALETTE["line"])
    sidebar.grid(row=0, column=0, sticky="ns")
    sidebar.grid_propagate(False)
    for index, (page_key, icon) in enumerate(SIDEBAR_NAV_ITEMS):
        size = 60 if index == 0 else 46
        canvas = ctk.CTkCanvas(sidebar, width=size, height=size, bg="#1C0709", highlightthickness=0)
    ...
    dots = ctk.CTkCanvas(sidebar, width=46, height=62, bg="#1C0709", highlightthickness=0)
