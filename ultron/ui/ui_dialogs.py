"""Dialog windows used by the ULTRON desktop UI."""
...
from ultron.health.health_center import apply_safe_health_fixes, build_health_center
from ultron.health.observability import build_runtime_event_snapshot
from ultron.memory import MemoryControlService, build_memory_health_report
...
from ultron.ui.settings_panel import SettingsPanelModel
...
class SettingsDialog(ctk.CTkToplevel):
    ...
    def _save(self):
        ...
        if result["status"] == "saved":
            self._status_label.configure(text=f"Saved non-secret settings to {result['path']}. Restart ULTRON for runtime-sensitive changes.")
        else:
...
class RuntimeLogDialog(ctk.CTkToplevel):
    def __init__(self, master):
        super().__init__(master)
        self.title("ULTRON Runtime Logs")
...
class ReadmeDialog(ctk.CTkToplevel):
    def __init__(self, master):
        ...
        ctk.CTkLabel(
            header,
            text="Project documentation is shown inside ULTRON so first-run setup stays polished.",
            font=font("ui", 11),
            text_color=TEXT_DIM,
        ).grid(row=1, column=0, padx=18, pady=(0, 16), sticky="w")
...
class HealthCenterDialog(ctk.CTkToplevel):
    def __init__(self, master):
        super().__init__(master)
        self.title("ULTRON Health Center")
...
class MemoryViewerDialog(ctk.CTkToplevel):
    def __init__(self, master):
        super().__init__(master)
        self.title("ULTRON Memory Center")
        self.geometry("1000x700")
        self.configure(fg_color=BG)
        self.grab_set()
        self._box = None
