...
# Make pyautogui optional for compatibility
try:
    import pyautogui
...
def handle_runtime_action(action: str, params: dict, context: dict) -> bool | None:
    ...
    if action == "screenshot":
        if not PYAUTOGUI_AVAILABLE:
            speak("Screenshot functionality is not available on this system, sir.")
            return False
    ...
    if action == "type_text":
        if not PYAUTOGUI_AVAILABLE:
            speak("Text typing functionality is not available on this system, sir.")
            return False
    ...
    if action == "press_key":
        if not PYAUTOGUI_AVAILABLE:
            speak("Key press functionality is not available on this system, sir.")
            return False
        key = params.get("key", "")
        if key:
            if "+" in key:
    ...
    if action == "mouse_click":
        if not PYAUTOGUI_AVAILABLE:
            speak("Mouse control functionality is not available on this system, sir.")
            return False
        x = params.get("x", 0)
        y = params.get("y", 0)
    ...
    if action == "scroll":
        if not PYAUTOGUI_AVAILABLE:
            speak("Scroll functionality is not available on this system, sir.")
            return False
    ...
    if action == "minimize_all":
        if not PYAUTOGUI_AVAILABLE:
            speak("Window control functionality is not available on this system, sir.")
            return False
