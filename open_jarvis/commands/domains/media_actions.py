...
def handle_media_action(action: str, params: dict, context: dict) -> bool | None:
    ...
    try:
        device_id = _active_device_id(spotify_client)
        if action == "spotify_play":
            ...
            spotify_client.volume(clamp_volume(params.get("level", 50)), device_id=device_id)
        elif action == "spotify_search":
            query = params.get("query", "")
            results = spotify_client.search(q=query, limit=1, type="track")
            tracks = results["tracks"]["items"]
            if tracks:
                spotify_client.start_playback(uris=[tracks[0]["uri"]], device_id=device_id)
            ...
            playlists = spotify_client.current_user_playlists(limit=50).get("items", [])
            match = next((playlist for playlist in playlists if query in playlist["name"].lower()), None)
            if match:
                spotify_client.start_playback(context_uri=match["uri"], device_id=device_id)
                speak(f"Playing your playlist {match['name']}, sir.")
            else:
                speak(f"I couldn't find a playlist called {params.get('query', '')} in your library, sir.")
